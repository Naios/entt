...
struct dynamic_component_traits: entt::component_traits<std::underlying_type_t<T>> {
    ...
    T &element_at(T *const page, const std::size_t pos) const {
        ...
        std::uintptr_t const offset = dynamic_size_ * entt::fast_mod(pos, base_trait::page_size);
    ...
    void swap_element(T &left, T &right) {
        ...
        ENTT_ASSERT(l != r, "Assuming that the elements to swap are not aliased!");
    ...
    void move_element(T &left, T &&right) {
        ...
        ENTT_ASSERT(l != r, "Assuming that the elements to move are not aliased!");
