...
class basic_storage
    ...
    [[nodiscard]] auto &element_at(const std::size_t pos) const {
        ...
        if constexpr(is_dynamically_sized_type_v) {
            return traits_type::element_at(page, pos);
        } else {
            return page[fast_mod(pos, traits_type::page_size)];
        }
    }
    ...
    auto assure_at_least(const std::size_t pos) {
        ...
        if constexpr(is_dynamically_sized_type_v) {
            return std::addressof(traits_type::element_at(page, pos));
        } else {
            return page + fast_mod(pos, traits_type::page_size);
        }
    }
...
class basic_storage<Entity, Entity, Allocator>
    : public basic_sparse_set<Entity, Allocator>,
      public component_traits<Entity> {
    using alloc_traits = std::allocator_traits<Allocator>;
